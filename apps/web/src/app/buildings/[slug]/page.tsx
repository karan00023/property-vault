...
export default async function BuildingPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    url: `https://propertyvault.local/buildings/${building.slug}`,
    geo: { "@type": "GeoCoordinates", longitude: building.coordinates[0], latitude: building.coordinates[1] }
  };
  ...
  return (
    <main className="profile-page">
      ...
      <section className="profile-content" id="overview">
        ...
        <aside className="fact-card"><span className="section-kicker">At a glance</span><dl><div><dt>Address</dt><dd>{building.address}</dd></div><div><dt>Floors</dt><dd>{building.floors ?? "Sourcing"}</dd></div><div><dt>Total units</dt><dd>{building.totalUnits?.toLocaleString() ?? "Sourcing"}</dd></div><div><dt>Manager</dt><dd>{building.propertyManager ?? "Sourcing"}</dd></div><div><dt>Availability</dt><dd><a href={building.availabilityUrl ?? building.leasingUrl} target="_blank" rel="noreferrer">Confirm with official leasing</a></dd></div><div><dt>Review status</dt><dd><span className="status-approved">Human approved</span></dd></div></dl></aside>
      </section>

      <section className="residence-section" id="residences"><span className="section-kicker">Residence mix</span><h2>Verified rent bands and residence types</h2>{building.unitMix.length ? <div className="residence-types">{building.unitMix.map((type) => <div key={`${type.label}-${type.sourceUrl}`}><strong>{type.label}</strong><span>{type.rentBand} · {type.sqftBand}</span><a href={type.sourceUrl} target="_blank" rel="noreferrer">Source</a></div>)}</div> : building.residenceTypes.length ? <div className="residence-types">{building.residenceTypes.map((type) => <div key={type}><strong>{type}</strong><span>Rent band not yet sourced</span></div>)}</div> : <div className="sourcing-state"><FileSearch size={24} /><h3>Unit mix sourcing in progress</h3><p>We will publish these details only after a human reviewer confirms the official source.</p></div>}</section>
      <section className="floorplan-section" id="floorplans"><div><span className="section-kicker light">Floor plan library</span><h2>{building.sourcing.floorPlanTasks ? "Sourcing in progress" : "Not yet available"}</h2><p>PropertyVault publishes only official PDFs, licensed images, or verified SightMap / Engrain embeds. Missing assets create sourcing tasks; nothing is drawn or invented.</p></div><CalendarClock size={34} /></section>
      <section className="provenance-section" id="provenance"><span className="section-kicker">Source record</span><h2>Why you can trust this page</h2><div className="source-card"><ShieldCheck size={24} /><div><strong>{building.provenance.sourceLabel}</strong><a href={building.provenance.sourceUrl} target="_blank" rel="noreferrer">{building.provenance.sourceUrl} <ExternalLink size={13} /></a></div><div><span>Confidence</span><strong>{building.provenance.confidence}</strong></div><div><span>Last fetched</span><strong>{fetched}</strong></div></div><div className="source-list">{building.sourceRecords.map((source) => <a href={source.sourceUrl} target="_blank" rel="noreferrer" key={`${source.sourceUrl}-${source.sourceLabel}`}>{source.sourceLabel}<span>{source.confidence}</span></a>)}</div></section>
    </main>
  );
}
